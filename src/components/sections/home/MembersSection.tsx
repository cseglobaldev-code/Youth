...
import { MemberCardLarge } from '@/components/common/MemberCardLarge';
...
function mapToCardProps(member: typeof MOCK_MEMBER & { id: string }) {
  return {
    name: member.name,
    ...
    <section className="bg-[#F2F7FF] py-[120px] px-[288px]">
      ...
      <div className="grid grid-cols-3 gap-6">
        {visibleMembers.map((member) => (
          <MemberCardLarge key={member.id} {...mapToCardProps(member)} className="h-[456.68px]" />
        ))}
      </div>
